...
public class SimpleTests {
	...
	// test that the high end of path 1 is connected to the low end of path
	// 2
	connectedPoint = link2.getConnectedPoint(highPoint);
	if (connectedPoint == path2.getLowpoint()) {
	    // System.out.println("Path 1 high point is connected to path 2 lowpoint as
	    // expected.");
	} else {
	...
	if (connectedPoint == path2.getLowpoint()) {
	    // System.out.println("StraightLink: Path 1 high point is connected
	    // to path 2
	    // low point as expected.");
	} else {
	...
	if (connectedPoint == path3.getLowpoint()) {
	    // System.out.println("TurnLink: Path 1 high point is connected to
	    // path 3 low
	    // point as expected.");
	} else {
	...
	if (connectedPoint == path2.getLowpoint()) {
	    // System.out.println("TurnLink (false): Path 1 high point is
	    // connected to path
	    // 2 low point as expected.");
	} else {
	...
	connectedPoint = link5.getConnectedPoint(highPoint);
	if (connectedPoint == path3.getLowpoint()) {
	    // System.out.println("TurnLink (true): Path 1 high point is
	    // connected to path 3
	    // low point as expected.");
	} else {
	...
	if (position.getPointA() == path3.getLowpoint() && position.getPointB().getPointIndex() == 1) {
	    // System.out.println("Position correctly moved to path 2.");
	} else {
	    System.out.println("Position not correctly moved to path 2.");
	}
	...
	// this next chunk of code goes IN BETWEEN the tests for multifixed and
	// multiswitch. this is bc calling switchConnection for link7 (the
	// multiswitch
	// link) changes what is connected to the high point of path1 (not
	// really sure
	// if this is right or if that's supposed to happen but that code was
	// given by
	...
	// this last chunk of code tests multiswitch and can just be pasted at
	// the end
	// of simpleTests:
